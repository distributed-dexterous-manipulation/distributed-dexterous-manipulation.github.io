...
class Transformer(nn.Module):
    def __init__(self, hp_dict, delta_array_size = (8, 8)):
        ...
        self.pos_embedding = SCE(self.max_agents, hp_dict['model_dim'])
        self.pos_embedding.load_state_dict(torch.load(hp_dict['sce'], map_location=self.device, weights_only=True))
    ...
    def compute_critic_loss(self, s1, a, s2, pos, rewards, d):
        ...
        with torch.no_grad():
            if self.gauss:
                ...
                next_q = self.decoder_critic(s2, next_actions, pos).squeeze()
                q_next = rewards + self.hp_dict['gamma'] * ((1 - d.unsqueeze(1)) * (next_q)).mean(dim=1)
