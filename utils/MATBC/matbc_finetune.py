...
from utils.MATBC.gpt_adaln_core import Transformer, count_vars
...
class MATBC_FT:
    def __init__(self, parent_hp_dict, logger):
        ...
        print(f"\nNumber of policy parameters: {np.sum(var_counts)}\n")
    ...
    def compute_q_loss(self, s1, a, s2, r, d, pos):
        ...
        with torch.no_grad():
            if self.gauss:
                ...
                next_q = self.tf_target.decoder_critic(s2, next_actions, pos).squeeze()
                q_next = r + self.hp_dict['gamma'] * ((1 - d.unsqueeze(1)) * (next_q - self.alpha * log_probs)).mean(dim=1)
            else:
                next_actions = self.tf(s2, pos)
                next_q = self.tf_target.decoder_critic(s2, next_actions, pos).squeeze()
                q_next = r + self.hp_dict['gamma'] * (1 - d.unsqueeze(1)) * (next_q).mean(dim=1)
    ...
    def compute_pi_loss(self, s1, pos):
        ...
        if self.hp_dict['learned_alpha']:
            ...
            alpha_loss = -(self.log_alpha * (log_probs - self.act_dim*n_agents).detach()).mean()
            alpha_loss.backward()
