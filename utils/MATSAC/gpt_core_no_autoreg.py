...
class GPT_AdaLN(nn.Module):
    ...
    def forward_normal(self, state, actions, pos, idx=None):
        ...
        if self.critic:
            conditional_enc = pos_embed.squeeze(2)
            act_enc = self.activation(self.action_embedding(actions) + state_enc)
        else:
            conditional_enc = pos_embed.squeeze(2) + state_enc
            act_enc = self.activation(self.action_embedding(actions))
...
class Transformer(nn.Module):
    def __init__(self, hp_dict, delta_array_size = (8, 8)):
        ...
        if hp_dict['pos_embed'] == "SPE":
            ...
            self.pos_embedding = SCE(self.max_agents, embedding_dim=256)
            self.pos_embedding.load_state_dict(
                torch.load("./utils/MATSAC/sce.pth",
        ...
        if hp_dict["attn_mech"] == "AdaLN":
            ...
            self.decoder_critic1 = GPT_AdaLN(hp_dict['state_dim'], hp_dict['model_dim'], self.action_dim, hp_dict['num_heads'], self.max_agents, hp_dict['dim_ff'], self.pos_embedding, hp_dict['dropout'], hp_dict['n_layers_dict']['critic'], mha, critic=True, masked=hp_dict['masked'])
            self.decoder_critic2 = GPT_AdaLN(hp_dict['state_dim'], hp_dict['model_dim'], self.action_dim, hp_dict['num_heads'], self.max_agents, hp_dict['dim_ff'], self.pos_embedding, hp_dict['dropout'], hp_dict['n_layers_dict']['critic'], mha, critic=True, masked=hp_dict['masked'])
            ...
            self.decoder_critic1 = GPT(hp_dict['state_dim'], hp_dict['model_dim'], self.action_dim, hp_dict['num_heads'], self.max_agents, hp_dict['dim_ff'], self.pos_embedding, hp_dict['dropout'], hp_dict['n_layers_dict']['critic'], mha, critic=True, masked=hp_dict['masked'], SA=sa_bool)
            self.decoder_critic2 = GPT(hp_dict['state_dim'], hp_dict['model_dim'], self.action_dim, hp_dict['num_heads'], self.max_agents, hp_dict['dim_ff'], self.pos_embedding, hp_dict['dropout'], hp_dict['n_layers_dict']['critic'], mha, critic=True, masked=hp_dict['masked'], SA=sa_bool)
